...
class States extends BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('country_id, status, deleted', 'numerical', 'integerOnly'=>true),
			array('id, created_by, modified_by', 'length', 'max'=>36),
			array('name', 'length', 'max'=>30),
			array('date_entered, date_modified', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, country_id, created_by, modified_by, status, deleted, date_entered, date_modified', 'safe', 'on'=>'search'),
		);
	}
